...
static void fail(const char *why) {
    u32 i = 0;
    while (why[i] && i < sizeof(T.error) - 1) { T.error[i] = why[i]; i++; }
    T.error[i] = 0;
}

const char *tls_error(void) { return T.error[0] ? T.error : "no error"; }
const char *tls_describe(void) { return T.described[0] ? T.described : ""; }
bool tls_active(void) { return T.open; }
bool tls_ended(void) { return T.ended && T.app_pos >= T.app_len; }

/* --- reading and writing whole things over TCP --------------------------- */

static bool read_exact(u8 *out, u32 n, u32 timeout_ms) {
    u32 have = 0;
...
static bool send_plain_record(u8 type, const u8 *data, u32 len) {
    u8 head[5] = { type, 0x03, 0x03, (u8)(len >> 8), (u8)len };
    if (!write_all(head, 5)) return false;
...
static bool send_encrypted(u8 inner_type, const u8 *data, u32 len) {
    ...
    u8 nonce[12];
    nonce_for(nonce, T.c_iv, T.c_seq);
    u8 tag[GCM_TAG];
    gcm_seal(&T.c_gcm, nonce, head, 5, buf, body, tag);
    T.c_seq++;
...
static bool read_record(u8 *type, u32 *len, bool encrypted) {
    u8 head[5];
    if (!read_exact(head, 5, 20000)) { fail("the connection stopped"); return false; }

    u32 n = ((u32)head[3] << 8) | head[4];
    if (n == 0 || n > REC_MAX) { fail("the server sent a record of an impossible size"); return false; }
    if (!read_exact(T.rec, n, 20000)) { fail("the connection stopped mid record"); return false; }
    ...
    if (n < GCM_TAG + 1) { fail("the server sent a record too short to be real"); return false; }

    u32 body = n - GCM_TAG;
    u8 nonce[12];
    nonce_for(nonce, T.s_iv, T.s_seq);
    if (!gcm_open(&T.s_gcm, nonce, head, 5, T.rec, body, T.rec + body)) {
        fail("a record did not authenticate");
        return false;
    }
    T.s_seq++;

    /* The real type is the last byte that is not padding. */
    while (body > 0 && T.rec[body - 1] == 0) body--;
    if (body == 0) { fail("a record had nothing in it"); return false; }
    ...
    *len = body - 1;
    ...
    if (*type == REC_ALERT) {
        ...
        if (*len >= 2 && T.rec[0] == 1) { T.ended = true; *len = 0; return true; }
        fail("the server closed the connection with an alert");
        return false;
    }
    return true;
}
...
static bool hs_fill(bool encrypted) {
    for (;;) {
        u8 type; u32 len;
        if (!read_record(&type, &len, encrypted)) return false;
        if (type == REC_CHANGE_CIPHER) continue;
        if (type != REC_HANDSHAKE) {
            fail("the server sent something other than a handshake message");
            return false;
        }
        if (T.hs_len + len > HS_MAX) { fail("the handshake is too big to hold"); return false; }
        memcpy(T.hs + T.hs_len, T.rec, len);
        T.hs_len += len;
        return true;
    }
}
...
/* One handshake message, with its four byte header, both of which the
   transcript needs. */
static bool hs_next(bool encrypted, u8 *type, const u8 **body, u32 *len,
                    const u8 **whole, u32 *whole_len) {
    for (;;) {
        u32 avail = T.hs_len - T.hs_pos;
        if (avail >= 4) {
            const u8 *p = T.hs + T.hs_pos;
            u32 n = ((u32)p[1] << 16) | ((u32)p[2] << 8) | p[3];
            if (n > HS_MAX) { fail("a handshake message claims an impossible size"); return false; }
            if (avail >= 4 + n) {
                *type = p[0];
                *body = p + 4;
                *len = n;
        ...
        if (!hs_fill(encrypted)) return false;
    }
}

/* --- the key schedule ----------------------------------------------------- */

static bool set_keys(const u8 secret[32], bool ours) {
    u8 key[16], iv[12];
    if (!hkdf_expand_label(secret, "key", 0, 0, key, 16)) return false;
    if (!hkdf_expand_label(secret, "iv", 0, 0, iv, 12)) return false;

    if (ours) {
        if (!gcm_init(&T.c_gcm, key, 128)) return false;
        memcpy(T.c_iv, iv, 12);
        T.c_seq = 0;
    } else {
        if (!gcm_init(&T.s_gcm, key, 128)) return false;
        memcpy(T.s_iv, iv, 12);
        T.s_seq = 0;
    }
    return true;
...
static bool parse_server_hello(const u8 *p, u32 len) {
    ...
    if (len < 38) { fail("the server hello is too short"); return false; }
    u32 i = 2;                                  /* the legacy version */

    if (memcmp(p + i, RETRY, 32) == 0) {
        fail("the server wants a key exchange group this does not have");
        return false;
    }
    i += 32;

    u32 sid = p[i++];
    if (i + sid + 3 > len) { fail("the server hello is malformed"); return false; }
    i += sid;

    u32 suite = ((u32)p[i] << 8) | p[i + 1]; i += 2;
    if (suite != SUITE_AES128_GCM_SHA256) {
        fail("the server chose a cipher this does not have");
        return false;
    }
    i++;                                        /* the compression byte */

    if (i + 2 > len) { fail("the server hello has no extensions"); return false; }
    u32 ext_len = ((u32)p[i] << 8) | p[i + 1]; i += 2;
    if (i + ext_len > len) { fail("the server hello is malformed"); return false; }
    ...
    while (i + 4 <= end) {
        ...
        i += 4;
        if (i + n > end) { fail("an extension runs past the message"); return false; }

        if (kind == 0x002b) {
            if (n != 2) { fail("the version extension is malformed"); return false; }
            u32 v = ((u32)p[i] << 8) | p[i + 1];
            if (v != 0x0304) {
                fail("the server does not do TLS 1.3");
                return false;
            }
            got_version = true;
        } else if (kind == 0x0033) {
            if (n < 4) { fail("the key share is malformed"); return false; }
            u32 group = ((u32)p[i] << 8) | p[i + 1];
            u32 klen = ((u32)p[i + 2] << 8) | p[i + 3];
            if (group != GROUP_X25519 || klen != 32 || n != klen + 4) {
                fail("the server chose a key exchange this does not have");
                return false;
            }
            memcpy(T.peer_share, p + i + 4, 32);
            got_share = true;
        }
        i += n;
    }
    ...
    if (!got_version) {
        ...
        fail("the server did not agree to TLS 1.3");
        return false;
    }
    if (!got_share) { fail("the server sent no key share"); return false; }
    return true;
}

/* --- what the server signs ------------------------------------------------ */

static bool check_cert_verify(const u8 *p, u32 len, const x509_t *leaf,
                              const u8 hash_before[32]) {
    if (len < 4) { fail("the signature message is too short"); return false; }
    u32 scheme = ((u32)p[0] << 8) | p[1];
    u32 sig_len = ((u32)p[2] << 8) | p[3];
    if (4 + sig_len != len) { fail("the signature message is malformed"); return false; }
    const u8 *sig = p + 4;
    ...
    switch (scheme) {
        ...
        default:
            fail("the server signed with something this cannot check");
            return false;
    }

    fail("the server's signature over the handshake is wrong");
    return false;
}
...
static bool alloc_buffers(void) {
    if (!T.rec) T.rec = (u8 *)kmalloc(REC_MAX + 64);
    if (!T.hs)  T.hs  = (u8 *)kmalloc(HS_MAX);
    if (!T.app) T.app = (u8 *)kmalloc(REC_MAX);
...
bool tls_connect(const char *host) {
    ...
    u32 i = 0;
    while (host[i] && i < sizeof(T.host) - 1) { T.host[i] = host[i]; i++; }
    T.host[i] = 0;
    if (!i) { fail("no host name to check a certificate against"); return false; }
    ...
    if (!rng_ready()) {
        fail("this machine has no source of randomness, so no key can be made");
        return false;
    }

    rng_bytes(T.priv, 32);
    rng_bytes(T.client_random, 32);
    rng_bytes(T.session_id, 32);
    x25519_public(T.priv, T.pub);

    sha256_init(&T.transcript);

    /* --- client hello --- */
    u8 *hello = T.rec;
    u32 hello_len = build_client_hello(hello + 4, REC_MAX - 4);
    if (!hello_len) { fail("could not build the greeting"); return false; }
    hello[0] = HS_CLIENT_HELLO;
    hello[1] = (u8)(hello_len >> 16);
    hello[2] = (u8)(hello_len >> 8);
    hello[3] = (u8)hello_len;

    sha256_update(&T.transcript, hello, hello_len + 4);

    {
        /* Copied out because the record buffer is about to be reused. */
        u8 *copy = (u8 *)kmalloc(hello_len + 4);
        if (!copy) { fail("not enough memory"); return false; }
        memcpy(copy, hello, hello_len + 4);
        bool sent = send_plain_record(REC_HANDSHAKE, copy, hello_len + 4);
        kfree(copy);
        if (!sent) { fail("could not send the greeting"); return false; }
    }

    /* --- server hello --- */
    {
        u8 type; const u8 *body, *whole; u32 len, whole_len;
        if (!hs_next(false, &type, &body, &len, &whole, &whole_len)) return false;
        if (type != HS_SERVER_HELLO) { fail("the server did not answer with a hello"); return false; }
        if (!parse_server_hello(body, len)) return false;
        sha256_update(&T.transcript, whole, whole_len);
    }

    /* --- the shared secret, and the keys for the rest of the handshake --- */
    u8 shared[32];
    if (!x25519(T.priv, T.peer_share, shared)) {
        fail("the server's key share is not usable");
        return false;
    }

    {
        ...
        if (!tls13_derive_secret(early, "derived", empty_hash, 32, derived)) {
            fail("the key schedule failed"); return false;
        }
        hkdf_extract(derived, 32, shared, 32, T.hs_secret);

        transcript_hash(th);
        if (!tls13_derive_secret(T.hs_secret, "c hs traffic", th, 32, T.c_hs) ||
            !tls13_derive_secret(T.hs_secret, "s hs traffic", th, 32, T.s_hs)) {
            fail("the key schedule failed"); return false;
        }
        if (!set_keys(T.s_hs, false)) { fail("could not set up decryption"); return false; }
    }
    ...
    if (!chain_store) { fail("not enough memory"); return false; }
    u32 chain_used = 0;

    for (;;) {
        u8 type; const u8 *body, *whole; u32 len, whole_len;
        if (!hs_next(true, &type, &body, &len, &whole, &whole_len)) return false;
        ...
        if (type == HS_ENCRYPTED_EXT) {
            ...
            if (len < 1) { fail("the certificate message is malformed"); return false; }
            u32 p = 0;
            u32 ctx = body[p++];
            if (p + ctx + 3 > len) { fail("the certificate message is malformed"); return false; }
            p += ctx;
            u32 list = ((u32)body[p] << 16) | ((u32)body[p + 1] << 8) | body[p + 2];
            p += 3;
            if (p + list > len) { fail("the certificate list is malformed"); return false; }

            u32 end = p + list;
            while (p + 3 <= end && nchain < X509_MAX_CHAIN) {
                u32 clen = ((u32)body[p] << 16) | ((u32)body[p + 1] << 8) | body[p + 2];
                p += 3;
                if (p + clen > end) { fail("a certificate runs past the list"); return false; }
                if (chain_used + clen > HS_MAX) { fail("the certificates are too big"); return false; }
            ...
            if (nchain == 0) { fail("the server sent no certificate"); return false; }
            if (!x509_parse(ders[0], lens[0], &leaf)) {
                fail("the server's certificate could not be read");
                return false;
            }
            have_leaf = true;
            sha256_update(&T.transcript, whole, whole_len);
            ...
            if (!have_leaf) { fail("a signature arrived before the certificate"); return false; }
            /* Over everything up to and including the certificate, so the
               hash is taken before this message joins the transcript. */
            transcript_hash(hash_before_verify);
            if (!check_cert_verify(body, len, &leaf, hash_before_verify)) return false;
            sha256_update(&T.transcript, whole, whole_len);
            ...
            u8 upto_here[32];
            transcript_hash(upto_here);

            u8 fkey[32], want[32];
            if (!hkdf_expand_label(T.s_hs, "finished", 0, 0, fkey, 32)) {
                fail("the key schedule failed"); return false;
            }
            hmac_sha256(fkey, 32, upto_here, 32, want);
            if (len != 32 || memcmp(want, body, 32) != 0) {
                fail("the server's finished message is wrong");
                return false;
            }
            sha256_update(&T.transcript, whole, whole_len);
            break;
            ...
            fail("the server sent an unexpected handshake message");
            return false;
        }
    }

    if (!have_leaf) { fail("the server never sent a certificate"); return false; }

    /* --- who it actually is ---------------------------------------------- */
    {
        u64 now = x509_now();
        x509_result_t r = x509_verify_chain(ders, lens, nchain, T.host, now);
        if (r != X509_OK) { fail(x509_reason(r)); return false; }
    }
    ...
    /* --- our finished, and then the keys for the conversation proper ----- */
    {
        ...
        if (!set_keys(T.c_hs, true)) { fail("could not set up encryption"); return false; }

        u8 fkey[32], mac[32], msg[36];
        if (!hkdf_expand_label(T.c_hs, "finished", 0, 0, fkey, 32)) {
            fail("the key schedule failed"); return false;
        }
        ...
        memcpy(msg + 4, mac, 32);
        if (!send_encrypted(REC_HANDSHAKE, msg, 36)) {
            fail("could not send the finished message");
            return false;
        }
        ...
        memset(zeros, 0, 32);
        sha256("", 0, empty_hash);
        if (!tls13_derive_secret(T.hs_secret, "derived", empty_hash, 32, derived)) {
            fail("the key schedule failed"); return false;
        }
        ...
        if (!tls13_derive_secret(master, "c ap traffic", th, 32, T.c_app) ||
            !tls13_derive_secret(master, "s ap traffic", th, 32, T.s_app)) {
            fail("the key schedule failed"); return false;
        }
        if (!set_keys(T.c_app, true) || !set_keys(T.s_app, false)) {
            fail("could not set up the connection keys");
            return false;
        }
        T.keys_are_app = true;
    }
    ...
    T.error[0] = 0;
    kformat(T.described, sizeof(T.described), "TLS 1.3, x25519, aes-128-gcm");
    bb_log("tls %s to %s", T.described, T.host);
    return true;
}
...
bool tls_send(const void *data, u32 len) {
    ...
    while (len) {
        ...
        if (!send_encrypted(REC_APPDATA, p, take)) return false;
        p += take;
        len -= take;
    }
    return true;
...
u32 tls_recv(u8 *out, u32 cap, u32 timeout_ms) {
    ...
    if (T.app_pos < T.app_len) {
        u32 have = T.app_len - T.app_pos;
        u32 take = have < cap ? have : cap;
        memcpy(out, T.app + T.app_pos, take);
        T.app_pos += take;
        return take;
    }
    if (T.ended) return 0;
    ...
    for (;;) {
        ...
        memcpy(T.app, T.rec, len);
        T.app_len = len;
        T.app_pos = 0;

        u32 take = len < cap ? len : cap;
        memcpy(out, T.app, take);
        T.app_pos = take;
        return take;
    }
}
...
void tls_close(void) {
    if (T.open) {
        ...
        u8 alert[2] = { 1, 0 };
        send_encrypted(REC_ALERT, alert, 2);
    }
    T.open = false;
    T.handshake_done = false;
    T.ended = false;
    T.app_len = T.app_pos = 0;
    T.hs_len = T.hs_pos = 0;
}
